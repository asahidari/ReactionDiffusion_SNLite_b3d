...
def setup():
    ...
    class DiffReact(ct.Structure):
        ...
        def __init__(self, np, steps=500, seed=24):
            ...
            U, V = np.zeros((n+2, n+2, n+2), dtype=np.double), np.zeros((n+2, n+2, n+2), dtype=np.double)
            ...
            libBasicRD3d = npct.load_library('libBasic_RD_3d', os.path.dirname('/Path/to/library/directory/'))

            # Declare process function arguments and result types
            libBasicRD3d.process.argtypes = [
               ct.POINTER(RdStruct),
            ...
            libBasicRD3d.process.restype = ct.c_int
            ...
            # Call C function
            res = libBasicRD3d.process(rd_struct, U.reshape(-1), V.reshape(-1), n, steps, cfunc_type(callback_func), ct.py_object(self))
